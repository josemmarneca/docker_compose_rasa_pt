...
class CustomChat(CustomBase):
    def chat_post_message(self, text, room_id=None, channel=None, **kwargs):
        ...
        if room_id:
            if text:
                return self.call_api_post(
                    "", roomId=room_id, text=text, kwargs=kwargs
                )
            return self.call_api_post("", roomId=room_id, kwargs=kwargs)
        if channel:
            if text:
                return self.call_api_post(
        ...
        raise CustomMissingParamException("roomId or addons required")

    def chat_send_message(self, message):
        if "rid" in message:
            return self.call_api_post("chat.sendMessage", message=message)
        raise CustomMissingParamException("message.rid required")
    ...
    def chat_search(self, room_id, search_text, **kwargs):
        """Search for messages in a addons by id and text message."""
    ...
    def chat_follow_message(self, mid, **kwargs):
        """Follows a chat message to the message's addons."""
